...
// Versão Digital
const int limite = 50; // Define limite do que é considerado ruído (apenas analógico)
...
void loop()
{
  ...
  if (barulho == HIGH) {
    ...
    // update the LED pin itself:
      ...
      tone(buzzer, 10); // Send 1KHz sound signal... default - 1000 / 1KHz - use 100 for testing.
      delay(100);        // ...for 1 sec
      noTone(buzzer);     // Stop sound...
    
    // send the string "Silence!" back to the computer, followed by newline
    Serial.println("Miúdos! Silêncioooooooooooo!");
    delay(2000);  // delay to avoid overloading the serial port buffer - default - 3000
  }
